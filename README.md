...
A **time-driven** tick fires at a fixed rate. The simulator from [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html) runs at 30 Hz: one tick every 33 ms. The loop wakes up, advances every system by one step, sleeps until the next tick. Most simulations, games, control loops, audio engines, and animation systems are time-driven. The rate is a contract with the rest of the world: at this rate, output appears.
...
A **filter** is 1→{0, 1}: every input row produces zero or one output rows. `apply_starve` (from [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html)) is a filter - each creature with `energy ≤ 0` produces an entry in `to_remove`; creatures with `energy > 0` produce nothing. The numpy form is one line:
...
The simulator's tick from [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html):
...
1. **Draw the DAG.** Take the eight simulator systems (motion, food_spawn, next_event, apply_eat, apply_reproduce, apply_starve, cleanup, inspect) and draw the dependency graph yourself, deriving the edges from each system's read-set and write-set in [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html). Compare with the diagram above.
...
- *History matters.* The simulator's `eaten`, `born`, `dead` logs from [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html) are all append-only - they record what happened. Removed entries would be lost history.
...
4. **Find your phase boundaries.** Sketch your simulator's full DAG from [`code/sim/SPEC.md`](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html). Identify each *phase* (set of systems with no transitive dependency on each other). Each phase is a parallel batch; each boundary is a sync.
...
- **Extend the simulator.** The genetics and predator-prey extensions flagged in the [simulator spec](https://root-11.codeberg.page/intro-book-python/code/sim/SPEC.html) break new ground without leaving the framework you have already built; `sim2b.py`'s predator is the worked example.
