...
This chapter is about *finding the wall*. The fixes are techniques you already have: subscription tables so a system touches only its subset ([§26](26_subscription_tables.md)), working-set discipline ([§27](27_working_set_vs_cache.md)), spatial binning ([§28](28_proximity.md)), narrower dtypes, pre-sized buffers, batched cleanup. The chapter's job is to teach the reader to *measure* - to find which constant factors blew up.
...
- **Cache spillover.** A `creature` working set at 10K is 200 KB (L2-resident). At 1M it is 20 MB (L3-resident). Per-element time triples. Fix: narrower numpy dtypes, and let each system read only the columns it needs - SoA already separates them ([§26](26_subscription_tables.md)/[§27](27_working_set_vs_cache.md)).
...
The fix is structural. Apply the techniques: subscription tables, working set, spatial binning, narrower dtypes, pre-sized buffers, batched cleanup, deterministic structures. Each is a chapter you have already read. The wall is the moment they all become non-optional.
...
6. **The unused column.** Add a column motion never reads and re-profile. The profile should not change - in numpy SoA each column is its own array ([§26](26_subscription_tables.md)), so motion never loads what it does not read. The working-set levers here are narrower dtypes and fewer columns per system, not a split. (If the simulator uses a numpy *structured array*, the unused field does cost you, and the fix is to move to SoA columns.)
...
[§30 - Moving beyond the wall](30_streaming_wall.md) takes the next step: when even your fastest, tightest, sorted-for-locality simulator no longer fits in RAM, the architecture itself shifts.
