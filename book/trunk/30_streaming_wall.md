...
**sqlite.** When the data is queried by id, range, or join — the access patterns relational databases were built for — sqlite is the right backend. From [§29](29_wall_10k_to_1m.md) and [`code/measurement/sqlite_performance_test.py`](https://github.com/root-11/intro-book-python/blob/main/code/measurement/sqlite_performance_test.py): ~830K-900K random lookups per second on disk, indistinguishable from memory at the level of a tick budget. The simulator's archive can be a sqlite database with one table per column-family; queries are `SELECT * FROM events WHERE tick BETWEEN ? AND ?`.
